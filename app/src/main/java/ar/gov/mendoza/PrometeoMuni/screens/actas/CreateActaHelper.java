...
public class CreateActaHelper
{
	   ...
	   public static void printTestigo(ZebraPrinter pPrinter)
	    throws ConnectionException
	   {
		   printer = pPrinter;
...
//              StringBuilderForPrint dataToPrint = new StringBuilderForPrint();
//              dataToPrint.append("! U1 LMARGIN 20 ")
//   			  .append("! U1 SETLP 7 0 10 " +  TextoAImprimir);
//              
//              return dataToPrint.toString();
	   }
	
	   public static void printTestImpresionCPCL(Connection pConnection,ZebraPrinter pPrinter)
	   throws ConnectionException
	   {
		   printer = pPrinter;
...
//	       Charset charset=Charset.forName("ISO-8859-2");
	   ...
	   public static void printCreateActaCPCL(ActaConstatacion data, ZebraPrinter pPrinter)
			     throws ConnectionException
			   {
			     printer = pPrinter;
				 byte[] fotoLicencia = data.getFotoLicencia();
				 byte[] fotoDocumento = data.getFotoDocumento();
				 Bitmap bitmap;
				 /*
				 ByteArrayInputStream imageStream = new ByteArrayInputStream(fotoLicencia);
				 BitmapFactory.Options outBitmap = new BitmapFactory.Options();
				 outBitmap.inJustDecodeBounds = false; // the decoder will return a bitmap
				 
				 Bitmap bitmap= BitmapFactory.decodeStream(imageStream,null,outBitmap);
				 //Bitmap bitmap = BitmapFactory.decodeByteArray(fotoLicencia , 0, fotoLicencia.length);
			     if (bitmap != null) {
			       printSignatureCPCL(bitmap);
			     }*/
			    
			     /*try
			     {
			     java.io.InputStream is = GlobalStateApp.getInstance().getApplicationContext().getContentResolver().openInputStream(uUri);
			     bitmap= BitmapFactory.decodeStream(is, null, outBitmap);
			     is.close();
			     }
...
/*			     printer.sendCommand(generateFormatCPCL0());
			     try
			     {
			    	 bitmap = ImageHelper.getBitmap(sUri);
			    	 if (bitmap != null) {
					       printSignatureCPCL(bitmap);
					     }
			     }
			     ...
			     printer.sendCommand("PRINT");
			    return;*/
				 Boolean bUsarImpresionConFotos= GlobalVar.getInstance().getSuportTable().getUsarImpresionConFotos() == "S";
			     if(bUsarImpresionConFotos==true)
			     {
			    	 printer.sendCommand(generateActaAndroidParte1FormatCPCL(data));

					 String[] sUri = data.getFotos().split("\\|");
				     //Uri uUri = Uri.parse( sUri[0]);
			    	 try
				     { 
				    	 bitmap = ImageHelper.getBitmap(sUri[0]);
				    	 if (bitmap != null) {
						       printLicenciaCPCL(bitmap);
						     }
				     }
				     catch(Exception ex)
				     {
			    	 ...
			    	 try
				     {
				    	 bitmap = ImageHelper.getBitmap(sUri[1]);
				    	 if (bitmap != null) {
						       printDocumentoCPCL(bitmap);
						     }
				     }
				     catch(Exception ex)
				     {
			    	 ...
			    	 //falta de Observaciones en adelante
			    	 printer.sendCommand("PRINT");
			    	...
			    	//Wait 850 milliseconds  
			    	 ...
			    	 {}
			    	 printer.sendCommand(generateActaParte2FormatCPCL(data));
					 printer.sendCommand("PRINT");
					 printer.sendCommand(generateActaParte3FormatCPCL(data));
					 printer.sendCommand("PRINT");
					 printer.sendCommand(generateActaParte4FormatCPCL(data));
					 printer.sendCommand("PRINT");
					 printer.sendCommand(generateActaParte5FormatCPCL(data));
					 printer.sendCommand("PRINT");
			     ...
			     {  
					 ...
					 printer.sendCommand(generateActaParte1FormatCPCL(data));
					 printer.sendCommand("PRINT");
					 printer.sendCommand(generateActaParte2FormatCPCL(data));
					 printer.sendCommand("PRINT");
				     
					 Boolean bPrintCupon= true;
					 if (bPrintCupon)
					 {
						 printer.sendCommand(generateActaParte3FormatCPCL(data));
						 printer.sendCommand("PRINT");
	   ...
	   private static String generateActaAndroidParte1FormatCPCL(ActaConstatacion data)
	   {
	     ...
	     //Comienzo impresion
           ...
           dataToPrint.append("! 0 200 200 1400 1")
           			  ...
           			  .append("PW 800")
           			  .append("LMARGIN 20")		//
           			  .append("T 7 1 20 0 PROVINCIA  DE  mdza")
           			  .append("T 7 1 20 33 Ministerio de Seguridad Publica")
           			  .append("T 7 0 533 57 " + acta.getNumeroActa())
           			  .append("T 7 0 20 70 Dir.  de Prevencion de")
           			  .append("T 7 0 20 90 Accidentes de Transito                  T.O. Ley 8.560")
           			  .append("B I2OF5 1 1 50 500 0 " + acta.getNumeroActa())
	   ...
	   private static String generateActaParte5FormatCPCL(ActaConstatacion data)
	   {
           ...
           dataToPrint.append("T 7 1 100 25 CUPON DE PAGO PARA EL BANCO - (CON DESCUENTO)").append("\r\n");
           ...
           dataToPrint.append("T 7 0 20 150 Importe de la Sancion....................... $    ").append("\r\n");
	   ...
	   private static String generateActaParte4FormatCPCL(ActaConstatacion data)
	   {
		 ...
		 //p_PrinterController.Print(dataToPrint.toString(), false);
           ...
           */
           ...
           dataToPrint.append("T 7 1 100 25 CUPON DE PAGO PARA EL INFRACTOR - (CON DESCUENTO)").append("\r\n");
           dataToPrint.append("T 7 0 20 75 Referencia Nro.Acta: " + acta.getNumeroActa()).append("\r\n");
           dataToPrint.append("T 7 0 580 75 Vto. " + Tools.DateValueOf(acta.getFechaVencimientoCupon(), "dd/MM/yyyy")).append("\r\n");
           dataToPrint.append("T 7 0 20 100 N.Doc. " + acta.getNumeroDocumento()).append("\r\n");
              ...
              // if (Global.GlobalFunc.esCodigoBarraKolektor(acta.Cod_barra))
               {
            	   ...
            	   dataToPrint.append("T 7 0 20 0 El pago del presente CUPON CON DESCUENTO podra efectuarse en").append("\r\n");

            	   dataToPrint.append("T 7 0 20 30 el Bco. de mdza, Rapipago o Cobro Express.").append("\r\n");
                   dataToPrint.append("T 7 0 20 60 Recuerde que puede presentar su descargo escrito dentro de ").append("\r\n");

                   dataToPrint.append("T 7 0 20 90 los 10 dIas habiles de labrada la presente acta, ofreciendo").append("\r\n");
                   ...
                   dataToPrint.append("T 7 0 20 225 Loc.: " + Tools.formatearDerecha(acta.getLocalidadJuzgado(), 25, ' ') + " PROVINCIA DE mdza").append("\r\n");

                   dataToPrint.append("T 7 0 20 265 Una vez vencido el plazo de pago, debera solicitar un nuevo").append("\r\n");

                   dataToPrint.append("T 7 0 20 295 cupon  en los Juzgados de Falta, en cualquier delegacion  ").append("\r\n");

                   dataToPrint.append("T 7 0 20 325 de la DGR de mdza. o a traves de www.dgrcba.gov.ar").append("\r\n");

                   dataToPrint.append("T 7 0 20 355 Ante el incumplimiento del pago en tiempo y forma se iniciaran").append("\r\n");
                   dataToPrint.append("T 7 0 20 385 las correspondientes acciones legales, sometiendose las partes").append("\r\n");

                   dataToPrint.append("T 7 0 20 415 a la juridiccion de los Tribunales Ordinarios de mdza,").append("\r\n");

                   dataToPrint.append("T 7 0 20 445 renunciando expresamente a cualquier fuero de excepcion que").append("\r\n");
	   ...
	   private static String generateActaParte2FormatCPCL(ActaConstatacion data)
	   {
           ...
           dataToPrint.append("T 7 0 40 160 Firma y aclaracion").append("\r\n");

           dataToPrint.append("T 7 0 480 160 Firma y aclaracion ").append("\r\n");
           ...
           */
           ...
           dataToPrint.append("T 7 1 190 295 QUE CONFECCIONO LA PRESENTE ACTA! ").append("\r\n");
	   ...
	   private static String generateActaParte1FormatCPCL(ActaConstatacion data)
	   {
	     ...
	     //Comienzo impresion
           ...
           dataToPrint.append("T 7 1 20 0 PROVINCIA  DE  CHACO\r\n");
           dataToPrint.append("T 7 1 20 33 Ministerio de Seguridad Publica\r\n");
           dataToPrint.append("T 7 0 533 57 " + acta.getNumeroActa()).append("\r\n");

           dataToPrint.append("T 7 0 20 70 Dir.  de Prevencion de \r\n");

           dataToPrint.append("T 7 0 20 90 Accidentes de Transito                  T.O. Ley 8.560\r\n");
           ...
           dataToPrint.append("T 7 0 20 160 Cod.Seg.Lic: " + acta.getNumeroLicencia() + " U.Prov:" + strUnicaProv + " Clase:" + acta.getClaseLicencia() + "  Vto:" + Tools.DateValueOf(acta.getFechaVencimientoLicencia(),"dd/MM/yy") + " Retenida: " + strRetenida).append("\r\n");
           																	//PaisActual																//strProvLic

           dataToPrint.append("T 7 0 20 185 Pais: " + Tools.formatearDerecha(acta.getPaisLicencia(), 25, ' ') + "  Provincia: " + Tools.formatearDerecha(acta.getProvinciaLicencia(), 25, ' ')).append("\r\n");
           																	//dptoLicencia																		//localidadActual	
           dataToPrint.append("T 7 0 20 210 Dpto.: " + Tools.formatearDerecha(acta.getDepartamentoLicencia(), 25, ' ') + "  Localidad: " + Tools.formatearDerecha(acta.getLocalidadLicencia(), 25, ' ')).append("\r\n");
           ...
           dataToPrint.append("T 7 0 20 325 Pais: " + Tools.formatearDerecha(acta.getPaisDomicilio(), 25, ' ')).append("\r\n");
           																	//strProvConductor		
           ...
           */
           ...
           if (acta.getCodigoInfraccion1()!=null && !acta.getCodigoInfraccion1().equals(""))
           {
               /*if(objInf1.ImprimirPuntos.Equals("S"))
                 ...
                 dataToPrint.append("T 7 0 20 480 Cod:" + acta.getCodigoInfraccion1() + " " + Tools.formatearDerecha(acta.getDescripcionInfraccion1(), 100, ' ').substring(0, 49)).append("\r\n");
                 														//objInf1.DescripcionCorta
               dataToPrint.append("T 7 0 20 505 " + Tools.formatearDerecha(acta.getDescripcionInfraccion1(), 100, ' ').substring(49, acta.getDescripcionInfraccion1().length())).append("\r\n");
           ...
           if (acta.getCodigoInfraccion2()!=null && !acta.getCodigoInfraccion2().equals(""))
           {
              /*if (objInf2.ImprimirPuntos.Equals("S"))
                   ...
                   dataToPrint.append("T 7 0 20 530 Cod:" + acta.getCodigoInfraccion2() + " " + Tools.formatearDerecha(acta.getDescripcionInfraccion2(), 100, ' ').substring(0, 49)).append("\r\n");
                   														//objInf2.DescripcionCorta								//47
               dataToPrint.append("T 7 0 20 555 " + Tools.formatearDerecha(acta.getDescripcionInfraccion2(), 100, ' ').substring(49, acta.getDescripcionInfraccion2().length())).append("\r\n");
           }
	   ...
	   private static void printDocumentoCPCL(Bitmap sig)
			     throws ConnectionException
			   {
				     int xPos = 10;
				     int yPos = 744 + 25;
	   ...
	   private static void printLicenciaCPCL(Bitmap sig)
	     throws ConnectionException
	   {
		   int xPos = 10;
		     int yPos = 160 +25;
